...
#include "AbilitySystem/Ability/DEPRECATED_UEntombedFireBolt.h"
...
FString UDEPRECATED_UEntombedFireBolt::GetLevelDescription(int32 Level)
{
...
FString UDEPRECATED_UEntombedFireBolt::GetNextLevelDescription(int32 Level)
{
	const int32 ScaledDamage = 0;
	const float Cost = FMath::Abs(GetCost(Level));
	const float Cooldown = GetCooldown(Level);
...
void UDEPRECATED_UEntombedFireBolt::SpawnProjectiles(const FGameplayTag& SocketTag, AActor* HomingTarget, bool bPitchOverride, float PitchOverride)
{
