...
#include "AbilitySystem/Ability/DEPRECATED_UEntombedBeamAbility.h"
...
void UDEPRECATED_UEntombedBeamAbility::StoreCursorDataInfo(const FHitResult& HitResult)
{
...
void UDEPRECATED_UEntombedBeamAbility::StoreOwnerVariables()
{
	if (CurrentActorInfo)
	{
...
void UDEPRECATED_UEntombedBeamAbility::TraceFirstTarget(const FVector& BeamTargetLocation)
{
	...
	{
		if (!CombatInterface->GetOnDeathDelegate().IsAlreadyBound(this, &UDEPRECATED_UEntombedBeamAbility::PrimaryTargetDead))
		{
			CombatInterface->GetOnDeathDelegate().AddDynamic(this, &UDEPRECATED_UEntombedBeamAbility::PrimaryTargetDead);
		}
	}
}

void UDEPRECATED_UEntombedBeamAbility::StoreAdditionalTargets(TArray<AActor*>& OutAdditionalTargets)
{
	TArray<AActor*> ActorsToIgnore;
	...
	for (AActor* AdditionalTarget : OutAdditionalTargets)
	{
		...
		{
			if (!CombatInterface->GetOnDeathDelegate().IsAlreadyBound(this, &UDEPRECATED_UEntombedBeamAbility::AdditionalTargetDead))
			{
				CombatInterface->GetOnDeathDelegate().AddDynamic(this, &UDEPRECATED_UEntombedBeamAbility::AdditionalTargetDead);
			}
		}
	}
}
